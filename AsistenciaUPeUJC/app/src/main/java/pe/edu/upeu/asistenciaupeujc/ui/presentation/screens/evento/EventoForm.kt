    ...
    Scaffold(modifier = Modifier.padding(top = 60.dp, start = 16.dp, end = 16.dp, bottom = 32.dp)){
        BuildEasyForms { easyForm ->
            Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
                ...
                ComboBox(easyForm = easyForm, "offlinex:", evento?.offlinex!!, listE)
                NameTextField(easyForms = easyForm, text =evento?.cui!!,"Nomb. Cui:", MyFormKeys.CUI )
                NameTextField(easyForms = easyForm, text =evento?.tipoCui!!,"Nomb. tipoCui:", MyFormKeys.TIPOCUI )
                NameTextField(easyForms = easyForm, text =evento?.materEntre!!,"mater_entre:", MyFormKeys.MATERENTRE )
                DatePickerCustom(easyForm = easyForm, label = "Fecha", texts = evento?.fecha!!, MyFormKeys.FECHA,"yyyy-MM-dd")

                TimePickerCustom(easyForm = easyForm, label = "hora_reg", texts = evento?.horaReg!!, MyFormKeys.HORAREG, "HH:mm:ss")
                DatePickerCustom(easyForm = easyForm, label = "modFh", texts = evento?.modFh!!, MyFormKeys.MODFH,"yyyy-MM-dd")
