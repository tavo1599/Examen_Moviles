...
class EventoFormViewModel @Inject constructor(
    private val event: EventoRepository,
    ...
    fun getInscritos(idX: Long): LiveData<Evento> {
        return event.buscarEventoId(idX)
    }
    ...
    fun addInscrito(evento: Evento){
        viewModelScope.launch (Dispatchers.IO){
            try {
                event.insertarEvento(evento)
            }catch (e:Exception){
    ...
    fun editEvento(evento: Evento){
        viewModelScope.launch(Dispatchers.IO){
            try {
                event.modificarRemoteEvento(evento)
            }catch (e:Exception){
