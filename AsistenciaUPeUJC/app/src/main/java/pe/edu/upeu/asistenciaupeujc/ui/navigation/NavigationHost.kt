...
import pe.edu.upeu.asistenciaupeujc.ui.presentation.screens.evento.EventoForm
import pe.edu.upeu.asistenciaupeujc.ui.presentation.screens.evento.EventoUI
...
fun NavigationHost(
    ...
    NavHost(
        ...
        composable(Destinations.EventoUI.route){
            EventoUI(navegarEditarAct = {newText->navController.navigate(Destinations.EventoForm.passId(newText))}, navController =navController )
        }
        composable(Destinations.EventoForm.route, arguments = listOf(navArgument("eventId"){
            defaultValue="eventId"
        })){
                navBackStackEntry -> var eventId=navBackStackEntry.arguments?.getString("eventId")
            requireNotNull(eventId)
            ActividadForm(text = eventId, darkMode = darkMode, navController =navController )
        }
